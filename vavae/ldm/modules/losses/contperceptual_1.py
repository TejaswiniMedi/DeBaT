...
def extract_high_freq(x, wave='haar'):
    # x: (B, C, H, W), should be divisible by 2
    dwt = DWTForward(J=1, mode='zero', wave=wave).to(x.device)  # single-level DWT
    Yl, Yh = dwt(x)  # Yl: low-freq, Yh: list of high-freq details at each level
    
    # Yh[0] is a tensor of shape (B, C, 3, H/2, W/2) — 3 for LH, HL, HH
    LH, HL, HH = Yh[0][:, :, 0], Yh[0][:, :, 1], Yh[0][:, :, 2]
    high_freq = LH + HL + HH  # Or just use HH if you want the finest details

    return high_freq  # shape: (B, C, H/2, W/2)


class LPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, kl_weight=1.0, pixelloss_weight=1.0,
                 disc_num_layers=3, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0,
                 perceptual_weight=1.0, high_freq_weight = 1.0,use_actnorm=False, disc_conditional=False,
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx,
        ...
        if not self.pp_style:
            ...
            # 🔹 Add High-Frequency Wavelet Loss
            if self.high_freq_weight > 0:

                hf_input = extract_high_freq(inputs)  #.detach().cpu().numpy()
                hf_recon = extract_high_freq(reconstructions)
                hf_loss = torch.abs(hf_input - hf_recon)
                hf_loss = torch.mean(hf_loss)
                rec_loss = rec_loss + self.high_freq_weight * hf_loss
        ...
        if optimizer_idx == 0:
            ...
            #print(loss.mean(), kl_loss.mean(), rec_loss.mean(), d_weight.mean(), g_loss.mean())
            return loss, log
